...
class LatentEditor(object):
    ...
    def apply_sefa(self, latent, indices=[2, 3, 4, 5], steps=5, **kwargs):
        edit_latents = sefa.edit(self.generator, latent, indices, **kwargs)
        return self._latents_to_image(edit_latents, column_limit=steps)
    ...
    def _latents_to_image(self, latents, column_limit=-1):
        with torch.no_grad():
        ...
        if column_limit == -1:
            ...
            for i in range(0, len(images_list), column_limit):
                horizontal_concat_image = torch.cat(images_list[i:i+column_limit], 2)
                if i == 0:
                    vertical_concat_image = horizontal_concat_image
                else:
                    vertical_concat_image = torch.cat([vertical_concat_image, horizontal_concat_image], 1)
            final_image = tensor2im(vertical_concat_image)
        return final_image
